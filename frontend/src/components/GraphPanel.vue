  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" :title="$t('graphPanel.refreshTooltip')">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? $t('graphPanel.graphMemoryUpdating') : $t('graphPanel.realtimeUpdating') }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">{{ $t('graphPanel.pendingContentHint') }}</span>
      ...
      <div v-else-if="loading" class="graph-state">
        ...
        <p>{{ $t('graphPanel.graphLoading') }}</p>
      </div>
      
      <!-- wait/Empty/NullStatus -->
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Changed from simulating to non-simulating state, show end hint
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    // Check if edge direction matches the normalized direction (sourceUUID < targetUUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Calculate curvature: spread out when multiple edges, straight line for single edge
    let curvature = 0
    if (totalCount > 1) {
      // Evenly distribute curvature, ensuring clear differentiation
      // Curvature range increases with edge count; more edges = larger curvature range
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // If edge direction is reversed from normalized direction, flip the curvature
      // This ensures all edges are distributed in the same reference frame without overlapping due to direction differences
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravitational pull toward center so isolated node groups cluster in the center area
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw a circular arc departing from and returning to the node
      const loopRadius = 30
    ...
    // Perpendicular offset to the link direction, calculated proportionally to distance, ensuring curves are clearly visible
    // More edges = larger offset-to-distance ratio
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // base25%，each additional edge adds5%
