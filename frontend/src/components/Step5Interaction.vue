            ...
            <div v-if="showToolsDetail" class="tools-card-body">
              <div class="tools-grid">
                ...
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">{{ $t('step5.tools.interviewSubAgent') }}</div>
                    <div class="tool-desc">{{ $t('step5.tools.interviewSubAgentDesc') }}</div>
                  </div>
                </div>
              </div>
            </div>
...
// Save current chat history到Cache
...
const selectReportAgentChat = () => {
  ...
  // Restore Report Agent 的对话Record
...
const selectAgent = (agent, idx) => {
  ...
  // Restore该 Agent 的对话Record
  chatHistory.value = chatHistoryCache.value[`agent_${idx}`] || []
  addLog(t('step5.log.selectedAgent', { name: agent.username }))
}
...
const sendMessage = async () => {
  ...
  try {
    ...
    addLog(t('step5.log.sendFailed', { error: err.message }))
    chatHistory.value.push({
      role: 'assistant',
      content: t('step5.log.errorResponse', { error: err.message }),
      timestamp: new Date().toISOString()
    })
  } finally {
    isSending.value = false
    scrollToBottom()
    // Autosave对话Record到Cache
    saveChatHistory()
  }
}

const sendToReportAgent = async (message) => {
  addLog(t('step5.log.sentToReportAgent', { msg: message.substring(0, 50) + '...' }))
  ...
  if (res.success && res.data) {
    chatHistory.value.push({
      role: 'assistant',
      content: res.data.response || res.data.answer || t('step5.log.noResponse'),
      timestamp: new Date().toISOString()
    })
    addLog(t('step5.log.reportAgentReplied'))
  } else {
    throw new Error(res.error || t('step5.log.requestFailed'))
  }
}

const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error(t('step5.log.selectAgentFirst'))
  }
  
  addLog(t('step5.log.sentToAgent', { name: selectedAgent.value.username, msg: message.substring(0, 50) + '...' }))
  ...
  if (res.success && res.data) {
    // 正确的DataPath: res.data.result.results 是一unitsObjectDict
    // Format: {"twitter_0": {...}, "reddit_0": {...}} 或单Platform {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // 将ObjectDictconvert为Array，优先get reddit Platform的回复
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // 优先Use reddit Platform回复，其次 twitter
    ...
    if (responseContent) {
      ...
      addLog(t('step5.log.agentReplied', { name: selectedAgent.value.username }))
    } else {
      throw new Error(t('step5.log.noResponseData'))
    }
  } else {
    throw new Error(res.error || t('step5.log.requestFailed'))
  }
...
const submitSurvey = async () => {
  ...
  addLog(t('step5.log.surveyStarted', { count: selectedAgents.value.size }))
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 将ObjectDictconvert为ArrayFormat
      ...
      for (const interview of interviews) {
        ...
        // 优先Use reddit Platform回复，其次 twitter
        let responseContent = t('step5.log.noResponse')
        ...
        if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
          ...
          if (agentResult) {
            responseContent = agentResult.response || agentResult.answer || t('step5.log.noResponse')
          }
        } else if (Array.isArray(resultsDict)) {
          // 兼容ArrayFormat
          const matchedResult = resultsDict.find(r => r.agent_id === agentIdx)
          if (matchedResult) {
            responseContent = matchedResult.response || matchedResult.answer || t('step5.log.noResponse')
          }
        }
      ...
      addLog(t('step5.log.surveyCompleted', { count: surveyResults.value.length }))
    } else {
      throw new Error(res.error || t('step5.log.requestFailed'))
    }
  } catch (err) {
    addLog(t('step5.log.surveyFailed', { error: err.message }))
  } finally {
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`loaded ${profiles.value.length} unitssimulationunits体`)
    }
  } catch (err) {
    addLog(`加载simulationunits体failed: ${err.message}`)
  }
}
...
/* 修复有序list编号 - use CSS 计数器让多units ol 连续编号 */
.message-text {
