    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? $t('step3.startingUp') : $t('step3.startGenerateReport') }}
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- generalRollback：unknownType或有 content 但未被上述process -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // fromStep2传入的最大rounds数
  minutesPerRound: {
    type: Number,
    default: 30 // default per round30minutes
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const phase = ref(0) // 0: 未开始, 1: 运行中, 2: 已completed
...
// Computed
// 按Time顺序show动作（最新的在finally面，i.e.底部）
...
// Format化Simulation流逝Time（根据rounds次和每roundsminutes数calculate）
...
// Reset所有State（forRe-启动Simulation）
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
const doStartSimulation = async () => {
  if (!props.simulationId) {
    addLog(t('step3.log.errorMissingSimId'))
    return
  }

  // 先Reset所有State，ensure不会受到上onceSimulation的影响
  resetAllState()

  isStarting.value = true
  startError.value = null
  addLog(t('step3.log.startingParallelSim'))
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(t('step3.log.setMaxRounds', { rounds: props.maxRounds }))
    }
    
    addLog(t('step3.log.graphUpdateEnabled'))
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog(t('step3.log.oldLogsCleared'))
      }
      ...
      startError.value = res.error || t('step3.log.startFailed')
      addLog(`✗ ${t('step3.log.startFailed')}: ${res.error || t('step3.log.unknownError')}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ ${t('step3.log.startError')}: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  addLog(t('step3.log.stoppingSim'))
  
  try {
    const res = await stopSimulation({ simulation_id: props.simulationId })
    
    if (res.success) {
      addLog(t('step3.log.simStopped'))
      phase.value = 2
      stopPolling()
      emit('update-status', 'completed')
    } else {
      addLog(t('step3.log.stopFailed', { error: res.error || t('step3.log.unknownError') }))
    }
  } catch (err) {
    addLog(t('step3.log.stopError', { error: err.message }))
  } finally {
    isStopping.value = false
  }
}
...
// Trace各Platform的上oncerounds次，for检测变化并outputLog
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 分别检测各Platform的rounds次变化并outputLog
      ...
      // 检测Simulation是否已Complete（through/via runner_status 或PlatformCompleteState判断）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // 额外check：if后端还没来得及Update runner_status，但PlatformalreadyReportComplete
      // through/via检测 twitter_completed 和 reddit_completed State判断
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog(t('step3.log.allPlatformsCompleted'))
        }
        addLog(t('step3.log.simCompleted'))
        phase.value = 2
        stopPolling()
        emit('update-status', 'completed')
      }
    ...
    console.warn('get运行statefailed:', err)
  }
}

// check所有enabled的Platform是否已Complete
const checkPlatformsCompleted = (data) => {
  // if没有任何PlatformData，return false
  if (!data) return false
  
  // check各Platform的CompleteState
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // if至少有一unitsPlatformComplete了，check是否所有enabled的Platform都Complete了
  // through/via actions_count 判断Platform是否被enabled（if count > 0 或 running 曾为 true）
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // if没有任何Platform被enabled，return false
  if (!twitterEnabled && !redditEnabled) return false
  
  // check所有enabled的Platform是否都已Complete
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 不Autoscroll，让用户自由查看Time轴
      // 新动作会在底部Append
    }
  } catch (err) {
    console.warn('get详细statefailed:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog(t('step3.log.reportAlreadySent'))
    return
  }

  isGeneratingReport.value = true
  addLog(t('step3.log.startingReport'))
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step3.log.reportTaskStarted', { id: reportId }))
      
      // Navigate toReportPage
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(t('step3.log.reportStartFailed', { error: res.error || t('step3.log.unknownError') }))
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(t('step3.log.reportStartError', { error: err.message }))
    isGeneratingReport.value = false
  }
}
