    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Export Buttons - 在Complete后Display -->
          <div v-if="isComplete" class="export-buttons">
            <button class="export-btn" @click="downloadReport('html')" :disabled="!isComplete">
            ...
            <button class="export-btn" @click="downloadReport('md')" :disabled="!isComplete">
              <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
                <polyline points="14 2 14 8 20 8"></polyline>
                <line x1="12" y1="18" x2="12" y2="12"></line>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (ContentgenerateComplete，但整unitssectionPossible还没Complete) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- 当是Final答案时，DisplayspecialHint/Tooltip -->
...
const downloadReport = (format) => {
  ...
  if (reportId) {
    window.open(`/api/report/${reportId}/download?format=${format}`, '_blank')
  }
}
...
const toggleRawResult = (timestamp, event) => {
  ...
  // etc.Pending DOM Update后，调整scroll位置以保持button在相同位置
...
const parseInsightForge = (text) => {
  ...
  try {
    // 提取Analysisissue
    const queryMatch = text.match(/analysisquestions:\s*(.+?)(?:\n|$)/)
    ...
    // 提取StatisticsData - 匹配"related预测事实: Xitems"Format
    ...
    // 提取子issue - 完整提取，不限制Quantity
    const subQSection = text.match(/### Analysis的子issue\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // 提取关键事实 - 完整提取，不限制Quantity
    ...
    // 提取CoreEntity - 完整提取，containsSummary和related事实数
    ...
    // 提取Relationshipchain - 完整提取，不限制Quantity
    const relSection = text.match(/### 【Relationshipchain】\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/当前valid事实:\s*(\d+)/)
    const histMatch = text.match(/historical\/expired事实:\s*(\d+)/)
    ...
    // 提取currentvalid事实 - 完整提取，不限制Quantity
    ...
    // 提取historical/expired事实 - 完整提取，不限制Quantity
    const histSection = text.match(/### 【historical\/expired事实】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // 提取Interview人数（如 "5 / 9 位SimulationAgent"）
    ...
    // 提取InterviewObjectSelect理由
    const reasonMatch = text.match(/### InterviewObjectselect理由\n([\s\S]*?)(?=\n---\n|\n### Interview实录)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Format1: Number. **名chars（index=X）**：理由
        ...
        // Format2: - Select名chars（index X）：理由
        // e.g.: - Select家长_601（index 0）：作为家长群体代Table...
        if (!headerMatch) {
        ...
        // Format3: - **名chars（index X）**：理由
        // e.g.: - **家长_601（index 0）**：作为家长群体代Table...
        if (!headerMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // if第一unitsissuefirst面有"1."，needspecialprocess
      ...
      if (answerMatch) {
        ...
        // PlatformRollback逻辑（兼容旧Format：只有一unitsPlatformmark/flag的情况）
        if (!twitterMatch && redditMatch) {
          // 只有 Reddit 回答，only在非占位text时复制为defaultshow
          ...
          // 没有分Platformmark/flag（极旧Format），整体作为回答
      ...
      if (quotesMatch) {
        ...
        // Rollback：匹配 > "text" 或 > \u201Ctext\u201D（Chinese引号）
        if (!quoteMatches) {
    ...
    // 提取InterviewSummary
    const summaryMatch = text.match(/### InterviewSummary与Core观点\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // 提取related事实 - 完整提取，不限制Quantity
    const factsSection = text.match(/### related事实:\n([\s\S]*)$/)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
    ...
    // 尝试提取边info（if有）
    const edgesSection = text.match(/### related边:\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          ...
          }, expandedFacts.value ? `收起 ▲` : `展开all ${props.result.facts.length} items ▼`)
        ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          ...
          }, expandedEntities.value ? `收起 ▲` : `展开all ${props.result.entities.length} units ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          ...
          }, expandedRelations.value ? `收起 ▲` : `展开all ${props.result.relations.length} items ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '漂移查询generatinganalysissub-questions'),
        ...
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, '暂无核心entity'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, '暂无relationship链')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `当前valid记忆 (${props.result.activeFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'historical' }],
          onClick: () => { activeTab.value = 'historical' }
        }, [
          h('span', { class: 'tab-label' }, `historical记忆 (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '当前valid记忆'),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '暂无涉及entity'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 支持两种编号Format：
      // 1. "issueX：" 或 "issueX:" （ChineseFormat，后端新Format）
      ...
      // if没匹配到，Rollback到 "Number." Format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // if没有found编号或只found一units，return整体
      if (matches.length <= 1) {
        const cleaned = answerText
          .replace(/^questions\d+[：:]\s*/, '')
          .replace(/^\d+\.\s+/, '')
          .trim()
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // splitSuccess且Indexvalid
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // splitFailed：第一unitsissuereturn完整回答，其余returnEmpty/Null
      return qIdx === 0 ? answer : ''
    }
    
    // check某unitsissue是否有双Platform回答（filter占位text）
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // 两unitsPlatform都有真实回答（非占位text）且Content不同
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // 双Platform切换button（only在有真实双Platform回答时show）
                  ...
                  // Expand/Collapse Button（占位text不show）
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '未found相关结果'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `收起 ▲` : `展开all ${props.result.facts.length} items ▼`)
...
// current活跃的步骤（for顶部show）
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // foundcurrent active 的步骤
  const active = steps.find(s => s.status === 'active')
  if (active) return active
  
  // if没有 active，returnfinally一units done 的步骤
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // 否则return第一units步骤
  return steps[0] || { noLabel: '--', title: '等pending开始', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // 去掉开头的二级Title（## xxx），becausesectionTitle已在外层show
  ...
  // cleanupList项之间的所有Empty/Null白
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // cleanupListStarttab/label后的Empty/Null白
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // cleanupListEndtab/labelfirst的Empty/Null白
  ...
  // process换行 - Empty/Null行变成段落分隔，单换行变成 <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // cleanup块级元素first后的 <br> tab/label
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // cleanup <p><br> 紧跟块级元素的情况（多余Empty/Null行导致）
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // cleanup连续的 <br> tab/label
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // cleanup块级元素后紧跟的段落Starttab/labelfirst的 <br>
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // 修复非连续有序List的编号：当单项 <ol> 被段落Content隔开时，保持编号递增
...
const getLogLevelClass = (log) => {
  ...
  // INFO Usedefault颜色，不mark/flag为 success
  return ''
}
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // scroll逻辑统一在循环End后的 nextTick 中process
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // ifTask已Complete，scroll到顶部；否则scroll到底部跟随最新Log
            if (isComplete.value) {
...
// 提取Final答案Content - from LLM response 中提取sectionContent
const extractFinalContent = (response) => {
  if (!response) return null
  
  // 尝试提取 <final_answer> tab/label内的Content
