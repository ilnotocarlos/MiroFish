<template>
  <div class="env-setup-panel">
    <div class="scroll-container">
      ...
      <!-- Step 02: generate Agent Persona/Profile -->
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">02</span>
            <span class="step-title">{{ $t('step2.generateAgentProfiles') }}</span>
          </div>
          <div class="step-status">
            <span v-if="phase > 1" class="badge success">{{ $t('common.status.completed') }}</span>
            <span v-else-if="phase === 1" class="badge processing">{{ prepareProgress }}%</span>
            <span v-else class="badge pending">{{ $t('common.status.waiting') }}</span>
          </div>
        </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.generatePlatformConfig') }}</span>
          </div>
          <div class="step-status">
            <span v-if="phase > 2" class="badge success">{{ $t('common.status.completed') }}</span>
            <span v-else-if="phase === 2" class="badge processing">{{ $t('common.status.generating') }}</span>
            <span v-else class="badge pending">{{ $t('common.status.waiting') }}</span>
          </div>
        </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        <div class="card-header">
          ...
          <div class="step-status">
            ...
            <span v-else-if="phase === 3" class="badge processing">{{ $t('common.status.processing') }}</span>
            <span v-else class="badge pending">{{ $t('common.status.waiting') }}</span>
          </div>
        </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        <div class="card-header">
          ...
          <div class="step-status">
            <span v-if="phase >= 4" class="badge processing">{{ $t('common.status.inProgress') }}</span>
            <span v-else class="badge pending">{{ $t('common.status.waiting') }}</span>
          </div>
        </div>
        ...
        <div class="card-content">
          ...
          <!-- Simulation round count config - only shown after config generation completes and round count is calculated -->
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">{{ $t('step2.profileModal.eventAge') }}</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} {{ $t('step2.profileModal.ageUnit') }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ $t('step2.profileModal.eventGender') }}</span>
              <span class="info-value">{{ { male: $t('step2.profileModal.genderMale'), female: $t('step2.profileModal.genderFemale'), other: $t('step2.profileModal.genderOther') }[selectedProfile.gender] || selectedProfile.gender }}</span>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
              <span class="info-label">{{ $t('step2.profileModal.eventMBTI') }}</span>
          ...
          <div class="modal-section">
            <span class="section-label">{{ $t('step2.profileModal.profileBio') }}</span>
            <p class="section-bio">{{ selectedProfile.bio || $t('step2.noBio') }}</p>
          </div>

          <!-- Followed topics -->
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">{{ $t('step2.profileModal.relatedTopics') }}</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">{{ $t('step2.profileModal.eventExperience') }}</span>
                <span class="dim-desc">{{ $t('step2.profileModal.eventExperienceDesc') }}</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">{{ $t('step2.profileModal.behaviorProfile') }}</span>
                <span class="dim-desc">{{ $t('step2.profileModal.behaviorProfileDesc') }}</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">{{ $t('step2.profileModal.uniqueMemory') }}</span>
                <span class="dim-desc">{{ $t('step2.profileModal.uniqueMemoryDesc') }}</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">{{ $t('step2.profileModal.socialNetwork') }}</span>
                <span class="dim-desc">{{ $t('step2.profileModal.socialNetworkDesc') }}</span>
              </div>
            </div>
...
// State
const phase = ref(0) // 0: initializing, 1: generate personas, 2: generate config, 3: completed
...
// Log去重：Record上onceoutput的关键info
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Simulationrounds数Config
const useCustomRounds = ref(false) // default use auto-configured rounds
const customMaxRounds = ref(40)   // default recommended40rounds
...
watch(currentStage, (newStage) => {
  if (newStage === 'generate Agent personas' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // EnterConfiggenerate阶段，Startrounds询Config
    if (!configTimer) {
      addLog(t('step2.log.startGeneratingConfig'))
      startConfigPolling()
    }
...
// fromConfig中calculateAutogenerate的rounds数（不Use硬EncodingdefaultValue）
const autoGeneratedRounds = computed(() => {
  ...
  // ensureMaxrounds数不小于40（RecommendedValue），avoid滑动items范围Exception
...
// processStartSimulationbutton点击
const handleStartSimulation = () => {
  // build传递给父Component的Parameter
  const params = {}
  
  if (useCustomRounds.value) {
    // 用户customrounds数，传递 max_rounds Parameter
...
// AutoStartprepareSimulation
const startPrepareSimulation = async () => {
  if (!props.simulationId) {
    addLog(t('step2.log.errorMissingSimId'))
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step2.log.taskId', { id: res.data.task_id }))
      
      // 立i.e.setExpectedAgent总数（fromprepareAPIreturnValueget）
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(t('step2.log.entitiesFromZep', { count: res.data.expected_entities_count }))
        if (res.data.entity_types && res.data.entity_types.length > 0) {
          addLog(t('step2.log.entityTypes', { types: res.data.entity_types.join(', ') }))
        }
      }
      
      addLog(t('step2.log.startPolling'))
      // Startrounds询Progress
      startPolling()
    ...
    addLog(t('step2.log.prepareError', { error: err.message }))
    emit('update-status', 'error')
  }
}
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 解析阶段info并outputDetailedLog
      if (data.progress_detail) {
        currentStage.value = data.progress_detail.current_stage_name || ''
        
        // outputDetailedProgressLog（avoid重复）
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 只有当 API returnvalidValue时才Update，avoidOverwrite已有的validValue
      ...
      // output Profile generateProgressLog（only当Quantity变化时）
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        addLog(`→ Agentpersona/profile ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || 'unknown profession'})`)
        
        // ifAllgenerateComplete
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ all ${currentCount} unitsAgent personas generated`)
        }
      }
    }
  } catch (err) {
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // outputConfiggenerate阶段Log（avoid重复）
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        lastLoggedConfigStage = data.generation_stage
        if (data.generation_stage === 'generating_profiles') {
          addLog('currentlygenerate Agent personas配置...')
        } else if (data.generation_stage === 'generating_config') {
          addLog('callingLLMgenerate simulation configparameters...')
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          ...
          addLog(`  ├─ hot topics: ${data.summary.hot_topics_count}units`)
        ...
        if (data.config.time_config) {
          ...
          addLog(`时间配置: 每rounds${tc.minutes_per_round}minutes, 共${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}rounds`)
        }
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ 环境搭建completed，canstart simulation')
...
const loadPreparedData = async () => {
  phase.value = 2
  addLog('currently加载已有配置data...')

  // finallygetonce Profiles
  await fetchProfilesRealtime()
  addLog(`已加载 ${profiles.value.length} Agent personas`)

  // getConfig（Usereal-timeAPI）
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // Confignot yetgenerate，Startrounds询
        addLog('配置generating中，开始rounds询等pending...')
        startConfigPolling()
      }
    }
  } catch (err) {
