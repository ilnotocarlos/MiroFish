...
from ..local_graph import LocalGraphClient, EpisodeData, EntityEdgeSourceTarget
...
class GraphBuilderService:
    ...
    def _build_graph_worker(
        ...
        try:
            self.task_manager.update_task(
                ...
                progress=5,
                message="Inizio costruzione grafo..."
            )
            ...
            self.task_manager.update_task(
                task_id,
                progress=60,
                message="In attesa dell'elaborazione Zep..."
            )
            ...
            self.task_manager.update_task(
                task_id,
                progress=90,
                message="Recupero informazioni grafo..."
            )
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # Local replacements for Zep ontology classes
        try:
            ...
            EntityText = Optional[str]  # Simple type compatible with Pydantic v2
    ...
    def add_text_batches(
        ...
        for i in range(0, total_chunks, batch_size):
            ...
            if progress_callback:
                ...
                progress_callback(
                    f"Invio batch {batch_num}/{total_batches} ({len(batch_chunks)} blocchi)...",
                    progress
                )
            ...
            try:
                ...
                if progress_callback:
                    progress_callback(f"Invio batch {batch_num} fallito: {str(e)}", 0)
                raise
    ...
    def _wait_for_episodes(
        ...
        if progress_callback:
            progress_callback(f"Inizio attesa elaborazione {total_episodes} blocchi di testo...", 0)

        while pending_episodes:
            if time.time() - start_time > timeout:
                if progress_callback:
                    progress_callback(
                        f"Timeout parziale blocchi testo, completati {completed_count}/{total_episodes}",
            ...
            if progress_callback:
                progress_callback(
                    f"Elaborazione Zep... {completed_count}/{total_episodes} completati, {len(pending_episodes)} in attesa ({elapsed}s)",
