"""
中文 LLM 提示词
Chinese LLM prompts - extracted from source files for i18n localization.
"""
...
INDIVIDUAL_PERSONA_PROMPT = """为实体生成详细的社交媒体用户人设,最大程度还原已有现实情况。
...
GROUP_PERSONA_PROMPT = """为机构/群体实体生成详细的社交媒体账号设定,最大程度还原已有现实情况。
...
TIME_CONFIG_SYSTEM_PROMPT = """你是社交媒体模拟专家。返回纯JSON格式，时间配置需符合中国人作息习惯。"""

TIME_CONFIG_PROMPT = """基于以下模拟需求，生成时间模拟配置。
...
EVENT_CONFIG_SYSTEM_PROMPT = """你是舆论分析专家。返回纯JSON格式。注意 poster_type 必须精确匹配可用实体类型。"""

EVENT_CONFIG_PROMPT = """基于以下模拟需求，生成事件配置。
...
AGENT_CONFIG_SYSTEM_PROMPT = """你是社交媒体行为分析专家。返回纯JSON，配置需符合中国人作息习惯。"""

AGENT_CONFIG_PROMPT = """基于以下信息，为每个实体生成社交媒体活动配置。
